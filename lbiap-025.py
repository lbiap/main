...
def lbiap_go():
	...
	print("Done")
	files = list(glob.glob(corpusfiles))
	...
	bundlelens = 9
	for filePath in glob.glob(corpusfiles ):
		fileInfo_d = getFileInfo( filePath )
		fileName = os.path.join(olddir, fileInfo_d["fileName"])
		print(fileName)
		folderPath = fileInfo_d["folderPath"]
	...
	filecount = df_final_results.count(0)
	...
	for (columnName, columnData) in df_final_results_exp.iteritems(): 
	...
	filecount = df_results_cleaned.count(0)
	filecount = filecount[1]
